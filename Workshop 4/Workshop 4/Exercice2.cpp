...
int StockageObjet2D::taille() {					// Si la liste est vide, la taille est 0
		if (Head == nullptr) {
		return 0;
	}											
	int taille = 1;						// Sinon la taille est égale au nombre d'éléments
...
void StockageObjet2D::SupHead() {											
	if (Head == nullptr) {									// Si la liste est vide, ne fait rien
		return;
	}										
	if (Head == Tail) {									// Si la liste ne contient qu'un élément, la tête et la queue sont mises à nullptr
		Head = nullptr;
		Tail = nullptr;
	}
	else {											// La tête devient le deuxième élément
...
void StockageObjet2D::SupTail() {
	...
	else {											// On parcourt la liste jusqu'au deuxième dernier élément
		Objet2D* precedent = Head;
		while (precedent->getSuivant()->getSuivant() != nullptr) {
			precedent = precedent->getSuivant();
		}										// Le deuxième dernier élément devient la nouvelle queue
		Tail = precedent;								// On supprime le dernier élément
		precedent->setSuivant(nullptr);
	}
}

void StockageObjet2D::SupIndex(int index) {							// Si l'index est négatif ou supérieur à la taille de la liste, ne fait rien
